...
The Hetzner Terraform module derives every one of them from a single variable so they cannot drift apart — set `TF_VAR_world_upload_max_size_mb` and nginx's cap, both services' multipart limits, and the wrapper's own check all move together. `world_upload_max_extracted_mb`, `world_upload_timeout_seconds`, `multipart_temp_dir`, and `mcserver_storage_size` are exposed alongside it. Note that the module aligns `nginx.maxBodySize` with the app-side limits rather than leaving it at the chart's `100M`, so a Hetzner deployment does not silently cap uploads below what the services advertise.
