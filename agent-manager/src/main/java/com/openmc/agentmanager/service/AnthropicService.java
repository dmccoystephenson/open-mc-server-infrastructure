...
public class AnthropicService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            - get_server_diagnostics: Gathers context from multiple sources and returns a structured JSON blob containing: server status, recent alerts, latest backup result, server performance metrics (JVM heap and TPS), and — when enabled — recent server logs and webapp data (player activity tracker stats including login counts and leaderboard). Use this tool whenever the user asks about any of these data sources, including but not limited to: "is the server okay?", "why is it lagging?", "what happened while I was offline?", "who has played the most?", "show me the activity leaderboard", "how many players have joined?", "when was the last backup?". Prefer this tool over get_server_status for any question that could be answered by its richer data set. The tool is always read-only. IMPORTANT: After receiving the JSON result, reply with ONLY the information relevant to what the user asked — do not dump all fields. Synthesize a concise natural language answer (e.g. if asked about the leaderboard, list the top players from activityTrackerStats; if asked about lag, focus on TPS and heap usage). If any source was unavailable, acknowledge the gap only if it is relevant to the user's question.
