...
public class AnthropicService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            - get_activity_tracker_stats: Fetches player activity statistics from the webapp: total/unique login counts and the player leaderboard. Use this when the user asks about player activity, who has played the most, or requests a leaderboard.
            - get_server_diagnostics: Gathers context from multiple sources in a single pass — server status, recent alerts, latest backup result, server performance metrics, and (when enabled) recent server logs and webapp activity stats. Use this for open-ended health questions such as "is the server okay?", "why is it lagging?", or "what happened while I was offline?". Prefer the more focused tools above (get_server_metrics, get_activity_tracker_stats) when the user's question maps clearly to a single data source. IMPORTANT: After receiving the JSON result, reply with ONLY the information relevant to what the user asked — do not dump all fields. If any source was unavailable, acknowledge the gap only if it is relevant to the user's question.
