...
public class AnthropicService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            - get_server_diagnostics: Gathers context from multiple sources (server status, recent alerts, latest backup, and optionally recent server logs when enabled) and returns a structured JSON blob. Use this tool — instead of get_server_status — when the user asks an open-ended health question such as "is the server okay?", "why is it lagging?", or "what happened while I was offline?". The tool is always read-only. After receiving the JSON result, synthesize a concise natural language summary that connects the data points (e.g. correlate a crash time with the last backup window, or flag suspicious log entries near a crash). If any source was unavailable, acknowledge the gap in your response.
